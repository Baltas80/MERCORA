...
function render(){const l=getLocale();if(language)language.value=l;applyTranslations(l)}
form?.addEventListener("submit",async e=>{
  e.preventDefault();
  const data=Object.fromEntries(new FormData(form));
  data.store_slug=normalize(data.store_slug);
  if(PREVIEW){fee.textContent="Demo activation fee: €0 · production pricing is not active.";message.textContent="PREVIEW — configuración validada visualmente. No se crea ninguna tienda ni pago.";return;}
...
language?.addEventListener("change",()=>{setLocale(language.value);render()});
render();