...
async function connect(){loginError.hidden=true;try{await request('GET','/api/admin/status');authenticated=true;passwordInput.value='';showDashboard();await refresh();refreshTimer=window.setInterval(refresh,15000)}catch(error){await invoke('admin_logout').catch(()=>{});authenticated=false;loginError.textContent=String(error?.message||'Authentication session could not be established.');loginError.hidden=false}}
async function refresh(){if(!authenticated)return;if(Date.now()-lastActivity>SESSION_IDLE_MS){await logout();loginError.textContent='Session expired due to inactivity.';loginError.hidden=false;return}try{render(await request('GET','/api/admin/status'))}catch(error){if(String(error.message||'').includes('401')){await logout();loginError.textContent='Session expired. Sign in again.';loginError.hidden=false;return}overall.textContent='OFFLINE';overall.className='badge bad';output.textContent=String(error.message||error)}}
