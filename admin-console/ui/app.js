...
async function connect(){loginError.hidden=true;try{await request('GET','/api/admin/status');authenticated=true;tokenInput.value='';showDashboard();await refresh();refreshTimer=window.setInterval(refresh,15000)}catch(error){await invoke('clear_token').catch(()=>{});loginError.textContent='Connection failed. Check the local Admin Control API and credential.';loginError.hidden=false}}
...
async function unlock(){loginError.hidden=true;unlockButton.disabled=true;try{const loaded=await invoke('load_stored_token');if(!loaded)throw new Error('No saved admin credential is available.');await connect()}catch(error){await invoke('clear_token').catch(()=>{});loginError.textContent=String(error.message||error);loginError.hidden=false}finally{unlockButton.disabled=false}}
async function bootstrap(){loginError.hidden=true;bootstrapButton.disabled=true;try{await invoke('bootstrap_token');await connect()}catch(error){loginError.textContent=String(error.message||error);loginError.hidden=false}finally{bootstrapButton.disabled=false}}
async function loadStoredCredential(){if(!invoke)return;try{const loaded=await invoke('load_stored_token');if(loaded)await connect()}catch(error){loginError.textContent=String(error.message||error);loginError.hidden=false}}
form.addEventListener('submit',async event=>{event.preventDefault();loginError.hidden=true;try{if(!tokenInput.value)throw new Error('Enter the configured admin token or initialize a local credential.');await invoke('set_token',{token:tokenInput.value});await connect()}catch(error){await invoke('clear_token').catch(()=>{});loginError.textContent=String(error.message||'Connection failed.');loginError.hidden=false}});
