...
async function logout(){authenticated=false;if(refreshTimer){window.clearInterval(refreshTimer);refreshTimer=null}await invoke?.('forget_token').catch(()=>{});dashboard.classList.add('hidden');login.classList.remove('hidden');tokenInput.value='';markActivity()}
