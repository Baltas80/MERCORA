...
function stateClass(state) {
  if (healthyStates.has(state)) return 'ok';
  if (badStates.has(state)) return 'bad';
  return 'unknown';
}
...
function render(status) {
  ...
  for (const name of services) {
    ...
    stateElement.className = `state ${stateClass(state)}`;
    stateElement.textContent = state;

    card.append(nameElement, stateElement);
    grid.appendChild(card);
  }

  const allGood = ['MERCORA', 'TOR', 'BACKEND', 'POSTGRESQL', 'HEALTH CHECKS']
    .every(key => healthyStates.has(String(values[key]).toUpperCase()));
...
async function request(method, path, body) {
  ...
  markActivity();
  return JSON.parse(await invoke('admin_request', {
    method,
    path,
...
async function refresh() {
  ...
  if (Date.now() - lastActivity > SESSION_IDLE_MS) {
    await logout();
    loginError.textContent = 'Session expired due to inactivity.';
