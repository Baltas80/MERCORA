...
async function rejectOversized(req, res) {
  ...
  if (Number.isFinite(contentLength) && contentLength > MAX_BODY) {
    res.writeHead(413); res.end(JSON.stringify({ error: 'request too large' })); req.resume(); return true;
  }
  return false;
}
...
async function readJson(req) {
  ...
  for await (const chunk of req) {
    body += chunk;
    if (Buffer.byteLength(body) > MAX_BODY) throw Object.assign(new Error('request too large'), { statusCode: 413 });
  }
...
export function createAdminApi({ controller, token, tokenHash, tokenFile = null, bootstrapPending = false, host = '127.0.0.1', port = 8787, username = process.env.MERCORA_ADMIN_USERNAME, passwordHash = process.env.MERCORA_ADMIN_PASSWORD_HASH } = {}) {
  ...
  if (!isValidAdminUsername(username) || typeof passwordHash !== 'string' || passwordHash.length < 40) {
    throw new Error('MERCORA_ADMIN_USERNAME and MERCORA_ADMIN_PASSWORD_HASH are required for password authentication');
  }
  ...
  function cleanup() {
    ...
    for (const [address, state] of failures) if (state.resetAt <= now) failures.delete(address);
  }
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/v1/login') {
      try {
        ...
        if (state && state.resetAt > now && state.count >= MAX_FAILED_LOGINS) {
          res.writeHead(429, { 'Retry-After': String(Math.ceil((state.resetAt - now) / 1000)) });
          return res.end(JSON.stringify({ error: 'too many login attempts' }));
        }
        const valid = input.username === username && verifyAdminPassword(input.password, passwordHash);
        if (!valid) {
          const current = state && state.resetAt > now ? state : { count: 0, resetAt: now + FAILED_WINDOW_MS };
        ...
        res.writeHead(status); return res.end(JSON.stringify({ error: status === 413 ? 'request too large' : 'invalid request' }));
      }
    }

    if (req.method === 'POST' && req.url === '/v1/logout') {
      const session = cookieSession(req); if (session) sessions.delete(session);
      res.setHeader('Set-Cookie', 'mercora_admin_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0');
    ...
    if (!authenticated(req)) {
      if (req.method === 'POST' && req.url === '/v1/bootstrap' && canBootstrap) {
        try {
          const bootstrapSecret = await claimBootstrapToken(tokenFile, token); canBootstrap = false;
          res.writeHead(200); return res.end(JSON.stringify({ token: bootstrapSecret }));
        } catch (error) {
          canBootstrap = false; res.writeHead(409); return res.end(JSON.stringify({ error: String(error.message ?? 'bootstrap unavailable') }));
        }
      }
      res.writeHead(401); return res.end(JSON.stringify({ error: 'unauthorized' }));
    }
    ...
    try {
      ...
      const status = error?.statusCode ?? 400; res.writeHead(status);
