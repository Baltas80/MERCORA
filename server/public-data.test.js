...
import { createPublicData, publicCategory, publicLimit, publicOffset, publicSearch, publicUuid } from './public-data.js';
...
test('public listing identifiers are strictly UUIDs',()=>{assert.throws(()=>publicUuid('bad','listing_id'),/UUID/);});
test('public catalog validates pagination',()=>{assert.equal(publicLimit(24),24);assert.equal(publicOffset(0),0);assert.throws(()=>publicLimit(0),/limit/);assert.throws(()=>publicOffset(-1),/offset/);});
test('public catalog validates search and category input',()=>{assert.equal(publicSearch('  camera  '),'camera');assert.equal(publicCategory('cameras-2026'),'cameras-2026');assert.throws(()=>publicSearch('x'.repeat(121)),/too long/);assert.throws(()=>publicCategory('bad category'),/format/);assert.throws(()=>publicSearch('bad\0value'),/invalid/);});
