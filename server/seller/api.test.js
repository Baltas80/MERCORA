...
test('seller rejects invalid account identifiers before database access',async()=>{
  const {runner,calls}=fakeRunner();
  const api=createSellerApi({databaseUrl:'postgresql://mercora:secret@postgres:5432/mercora',run:runner});
  await assert.rejects(()=>api.current('not-a-uuid'),/36-36|valid UUID/);
  assert.equal(calls.length,0);
});

test('seller API keeps database URL out of child arguments',async()=>{
  const {runner,calls}=fakeRunner();
  const api=createSellerApi({databaseUrl:'postgresql://mercora:secret@postgres:5432/mercora',run:runner});
  await api.current(ACCOUNT);
  assert.ok(calls.length>=1);
  assert.ok(calls.every(c=>!c.args.join(' ').includes('postgresql://')));
  assert.ok(calls.every(c=>c.args.at(-1)!=='DROP TABLE listings'));
  assert.equal(calls[0].options.env.PGPASSWORD,'secret');
});

test('listing creation rejects unsupported assets before INSERT',async()=>{
  const {runner,calls}=fakeRunner();
  const api=createSellerApi({databaseUrl:'postgresql://mercora:secret@postgres:5432/mercora',run:runner});
...
test('listing status changes are limited to owner and safe states',async()=>{
  const {runner,calls}=fakeRunner();
  const api=createSellerApi({databaseUrl:'postgresql://mercora:secret@postgres:5432/mercora',run:runner});
  const row=await api.setListingStatus(ACCOUNT,{listing_id:'44444444-4444-4444-8444-444444444444',status:'archived'});
