...
class EscrowStatus(StrEnum):
    HELD="held"; DISPUTED="disputed"; RELEASED="released"; REFUNDED="refunded"
...
class EscrowPolicy:
    ...
    def __post_init__(self):
        if self.version<=0 or self.successful_orders_required<0 or self.hold_after_delivery_seconds<=0: raise ValueError("invalid_escrow_policy")
    def requires_escrow(self,successful_orders:int)->bool:
        if successful_orders<0: raise ValueError("invalid_successful_orders")
...
def create_funded_escrows(cur,order_id:UUID)->int:
    cur.execute("""SELECT oi.seller_id,
                          SUM(oi.quantity*oi.unit_price_minor),
                          SUM(oi.seller_fee_minor),
                          o.currency
                   FROM order_items oi JOIN orders o ON o.id=oi.order_id
                   WHERE oi.order_id=%s GROUP BY oi.seller_id,o.currency""",(order_id,))
    count=0
    for seller_id,gross,total_seller_fee,currency in cur.fetchall():
        seller_fee=int(total_seller_fee or 0)
        net=int(gross)-seller_fee
        if net<0: raise ValueError("seller_escrow_negative")
        cur.execute("""INSERT INTO seller_escrows(id,order_id,seller_id,currency,amount_minor,policy_version,status)
                       SELECT gen_random_uuid(),%s,%s,%s,%s,ep.version,'held'
                       FROM escrow_policies ep WHERE ep.active=true
                       ON CONFLICT(order_id,seller_id) DO NOTHING""",(order_id,seller_id,currency,net))
        if cur.rowcount:
            cur.execute("""INSERT INTO escrow_ledger_entries(escrow_id,order_id,seller_id,currency,entry_type,amount_minor)
                           SELECT id,order_id,seller_id,currency,'hold',amount_minor
                           FROM seller_escrows WHERE order_id=%s AND seller_id=%s
                           ON CONFLICT(escrow_id,entry_type) DO NOTHING""",(order_id,seller_id))
            count+=1
    return count
