...
def calculate_order_amounts(lines: list[CartLine], policy: FeePolicy) -> OrderAmounts:
    ...
    seller_fee = sum(
        calculate_percentage_fee(line.subtotal_minor, policy.seller_fee_bps)
        for line in lines
    )

    return OrderAmounts(
        subtotal_minor=subtotal,
        buyer_fee_minor=buyer_fee,
        seller_fee_minor=seller_fee,
        total_minor=subtotal + buyer_fee,
    )


def prepare_order(lines: list[CartLine], policy: FeePolicy) -> PreparedOrder:
