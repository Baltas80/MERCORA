...
test('backup scheduler rejects unsafe frequency and retention settings', () => {
  ...
  assert.throws(() => parseBackupConfig({ MERCORA_BACKUP_RETENTION_DAYS: '0' }), /RETENTION_DAYS.*between/i);
  assert.throws(() => parseBackupConfig({ MERCORA_BACKUP_RETENTION_COUNT: '1' }), /RETENTION_COUNT.*between/i);
});
